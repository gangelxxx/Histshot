...
public partial class EditorWindow : Window
{
    ...
    public EditorWindow(CapturedImage capturedImage, int x, int y, int width, int height, double scaling)
    {
        ...
        Toolbar.CopyRequested += async (_, _) => await Editor.CopyAsync(Toolbar.Comment, Toolbar.ReminderAt);
        Toolbar.CancelRequested += (_, _) => Editor.Cancel();
        Toolbar.ReminderRequested += OnReminderRequested;
        Toolbar.LayoutUpdated += (_, _) => PositionToolbar();
    }

    private async void OnReminderRequested(object? sender, EventArgs e)
    {
        var dialog = new ReminderDialog(Toolbar.ReminderAt)
        {
        ...
        Toolbar.SetReminder(result.Remove ? null : result.When);
    }
