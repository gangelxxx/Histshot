...
- ✂️ **Area selection** — dims the screen so you can pick exactly what you want; works across multiple monitors with per-monitor DPI
- ✏️ **Built-in editor** — pencil, line, arrow, rectangle, and editable text; select, move and restyle any annotation afterwards
- 🎨 **Customizable tools** — color, line thickness, font size
- 💬 **Comments** — attach a note to a screenshot right at capture time, or add/edit it later in the history
- ⏰ **Reminders** — set a reminder on any capture and get a notification that jumps you back to that screenshot
- 🔍 **History search & cleanup** — search by comment text; bulk-delete items older than 1/3/7/14/30 days (or everything)
- 📋 **Copy or save** — straight to the clipboard or to a file on disk; one-click hover buttons copy the image or the comment text
- 🕘 **Screenshot history** — every capture is kept, nothing gets lost
- 🔄 **Auto-updates** — checks for new releases in the background (tray menu → Update)
- 🪶 **Light on resources** — trims its working set and drops GPU usage to zero when idle
...
3. Optionally draw an arrow, rectangle, or add text
4. Optionally write a comment or set a reminder right in the toolbar
5. Copy to the clipboard (`Ctrl + V` anywhere) or save it as a file

Every capture is automatically added to your **history** — nothing is lost, even if you forget to save.
In the history window you can search captures by comment text, edit comments, set reminders,
copy the image or its text with hover buttons, and clean up old items
(**Clear history** — older than 1/3/7/14/30 days or all at once).
